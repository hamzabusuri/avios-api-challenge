...
resource "aws_s3_bucket" "ng_beanstalk_deploys" {
  bucket = "${var.application_name}-deployments-${var.application_environment}"
}
...
resource "aws_iam_instance_profile" "ng_beanstalk_ec2" {
  name  = "pcs-user-${var.application_environment}"
...
resource "aws_iam_role_policy" "ng_beanstalk_ec2_policy" {
  name = "ng_beanstalk_ec2_policy_with_ECR-${var.application_environment}"
